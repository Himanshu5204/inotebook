...
router.post(
  ...
  async (req, res) => {
    ...
    const { title, description, tag, pinned } = req.body; // Destructure the request body to get title, description, and tag
    try {
      const note = new Note({ title, description, tag, pinned: !!pinned, user: req.user.id }); // user wise notes add + Create a new note instance
...
router.put('/updatenote/:id', fetchuser, async (req, res) => {
  ...
  if (typeof pinned !== 'undefined') newNote.pinned = pinned;
